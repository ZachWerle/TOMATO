...
def create_lateral_movement_matrix(graph: Graph) -> np.ndarray:
    """
    ...
    prob_matrix = np.empty((adjacency_matrix.shape[0], adjacency_matrix.shape[1]))
...
def create_on_host_tactic_matrix(graph: Graph) -> np.ndarray:
    """
    Creates an on-host tactic matrix.    
    """
    prob_matrix = np.zeros((graph.number_of_nodes(), graph.number_of_nodes()))
...
def create_probability_matrix(graph: Graph) -> np.ndarray:
    """
